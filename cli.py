...
3) Starts the conversation loop (02_main_loop.run_conversation_from_onboarding).
4) Evaluates the conversation using 03_evaluate_conversation.evaluate_conversation.
"""
import json, os, sys
...
SRC_EVAL    = THIS_DIR / "03_evaluate_conversation.py"
OUT_JSON    = THIS_DIR / "data" / "completes.json"
OUT_CONVO   = THIS_DIR / "data" / "conversation_log.json"
OUT_EVAL    = THIS_DIR / "data" / "evaluation.json"

def import_by_path(path: Path, module_name: str):
    import importlib.util, sys
    spec = importlib.util.spec_from_file_location(module_name, path)
    mod = importlib.util.module_from_spec(spec)
    sys.modules[module_name] = mod   # <-- important: register before exec
...
def main():
    ...
    OUT_CONVO.write_text(json.dumps(convo_result, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"[ok] Saved conversation log to: {OUT_CONVO}")

    # Import evaluator
    if not SRC_EVAL.exists():
        print(f"[error] Missing 03_evaluate_conversation.py at {SRC_EVAL}", file=sys.stderr); sys.exit(6)
    eval_mod = import_by_path(SRC_EVAL, "eval_mod")

    if not hasattr(eval_mod, "evaluate_conversation"):
        print("[error] 03_evaluate_conversation.py does not expose evaluate_conversation(convo_result, onboarding)")
        sys.exit(7)

    # Run evaluation
    evaluation = eval_mod.evaluate_conversation(convo_result, onboarding=data)
    # Persist
    eval_mod.save_evaluation(evaluation, OUT_EVAL)
    print(f"[ok] Wrote evaluation to: {OUT_EVAL}")
